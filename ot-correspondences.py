...
def parseArgs():
    parser = argparse.ArgumentParser(
        description="Generate point cloud correspondences using optimal transport. Author: Tim Golla <tim.golla.official@gmail.com>")
    ...
    for clusternr1 in range(np.max(labels[0]) + 1):
        ...
        if override:
            ...
            if args.solver == "ot.bregman.greenkhorn":
                ...
                G0, sinkhorntime = OptimalTransport.sinkhorn_sparse(
                    P, mass1, mass2)
            ...
            if userowmaxcolmax:
                ...
                G0_rowmax = sp.coo_matrix(
                    (rowmaxdata.toarray()[:, 0], (rowinds, rowargmaxes)), shape=G0.shape)
                assert(G0_rowmax.shape == G0.shape)
                G0_colmax = sp.coo_matrix(
                    (colmaxdata.toarray()[0], (colargmaxes, colinds)), shape=G0.shape)
        ...
        if usemax:
            ...
            for k in range(len(I)):
                ...
                if outputClusterColors:
                    ...
                    assignedcenter = np.hstack(
                        [centers[0][i][:3], centers[1][j][:3]])
                assignedcenters.append(assignedcenter)
...
def main():
    ...
    correspondenceIndices = solveAssignmentPredefinedClusters(
        vertices, labels, args)[0]
