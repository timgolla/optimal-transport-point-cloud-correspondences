...
def getKNNGraphsklearn(m1, m2, n_neighbors, algorithm='auto', n_jobs=-1, metric='euclidean'):
    ...
    nbrs12 = NearestNeighbors(
        n_neighbors, algorithm='auto', n_jobs=-1, metric='euclidean').fit(m2)
    ...
    costmatrix1 = sp.coo_matrix(
        (dists.flatten('C'), (i, nninds.flatten('C'))), shape=(len(m1), len(m2)))
    et = time.time()
    totaltime = et - st
